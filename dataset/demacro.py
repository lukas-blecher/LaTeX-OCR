...
def parse_command_line():
    ...
    parser.add_argument(
        "--output", "-o", required=True, help="TeX output file with \\newcommand"
    )
    parser.add_argument(
        "--demacro",
        action="store_true",
        help="replace all commands with their definition",
    )
...
def unfold(t):
    ...
    for c in cmds:
        ...
        if nargs == 0:
            ...
            t = re.sub(
                r"\\%s([\W_^\d])" % c[0], r"%s\1" % c[-1].replace("\\", r"\\"), t
            )
        else:
            matches = re.findall(
                r"(\\%s(?:\[(.+?)\])?" % c[0]
                + r"{(.+?)}" * (nargs - (1 if c[2] != r"" else 0))
                + r")",
                t,
            )
            # print(matches)
...
def replace(match):
    ...
    if prefix is not None and (
        "expandafter" in prefix
        or "global" in prefix
        or "outer" in prefix
        or "protected" in prefix
    ):
