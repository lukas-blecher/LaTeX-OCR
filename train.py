...
def train(args):
    ...
    def save_models(e):
        torch.save(
            model.state_dict(),
            os.path.join(args.out_path, "%s_e%02d.pth" % (args.name, e + 1)),
        )
        yaml.dump(dict(args), open(os.path.join(args.out_path, "config.yaml"), "w+"))

    opt = get_optimizer(args.optimizer)(model.parameters(), args.lr, betas=args.betas)
    scheduler = get_scheduler(args.scheduler)(
        opt, max_lr=args.max_lr, steps_per_epoch=len(dataloader) * 2, epochs=args.epochs
    )  # scheduler steps are weird.
    try:
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "-d",
        "--data",
        default="dataset/data/train.pkl",
        type=str,
